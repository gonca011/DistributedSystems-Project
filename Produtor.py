...
conexao = None  # Define conexão como uma variável global

parser = argparse.ArgumentParser(description='Start the producer server.')
parser.add_argument('--port', type=int, default=5004, help='Port number to run the producer server on.')
args = parser.parse_args()

host = "localhost"
port = args.port

# Função para carregar produtos de um ficheiro JSON
def load_produtos(file_path):
    with open(file_path, 'r') as file:
        return json.load(file)

# Função para guardar produtos num ficheiro JSON
def save_produtos(file_path, produtos):
    with open(file_path, 'w') as file:
        json.dump(produtos, file, indent=4)

# Descobrir a diretoria atual
script_dir = os.path.dirname(os.path.abspath(__file__))

# Caminho para o ficheiro JSON
produtos_file_path = os.path.join(script_dir, 'produtos.json')

# Carregar a Lista de Produtos do ficheiro
            ...
            for produto in produtos[categoria]:
                if produto['nome'].strip().lower() == produto_nome:
                    if produto['quantidade'] >= quantidade:
                        ...
                        save_produtos(produtos_file_path, produtos)  # Guarda
                        response = {
                            "status": "sucesso",
...
def iniciar_servidor(servidor_host, servidor_port):
    global conexao  # Usa a variável global
